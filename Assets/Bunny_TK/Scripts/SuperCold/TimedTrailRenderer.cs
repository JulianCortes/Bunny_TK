...
public class TimedTrailRenderer : MonoBehaviour
{
    ...
    void Update()
    {
        if (emit && emitTime != 0)
        {
            emitTime -= TimeManager.Instance.ScaledDeltaTime;
        ...
        if (emit)
        {
            if (theDistance > minVertexDistance)
            {
                ...
                if (make)
                {
                    ...
                    p.timeCreated = TimeManager.Instance.ScaledTime;
                    points.Add(p);
                    lastPosition = transform.position;
                }
                else
                {
                    ...
                    ((Point)points[points.Count - 1]).timeCreated = TimeManager.Instance.ScaledTime;
                }
            }
        ...
        if (points.Count > 1)
        {
            ...
            if (distance > movePixelsForRebuild || TimeManager.Instance.ScaledTime - lastRebuildTime > maxRebuildTime)
            {
                re = true;
        ...
        if (re)
        {
            lastRebuildTime = TimeManager.Instance.ScaledTime;
            ...
            foreach (Point p in points)
            {
                ...
                if (TimeManager.Instance.ScaledTime - p.timeCreated > lifeTime) remove.Add(p);
                i++;
            }
            ...
            if (points.Count > 1)
            {
                ...
                foreach (Point p in points)
                {
                    float ScaledTime = (TimeManager.Instance.ScaledTime - p.timeCreated) / lifeTime;

                    Color color = Color.Lerp(Color.white, Color.clear, ScaledTime);
                    if (colors != null && colors.Length > 0)
                    {
                        float colorTime = ScaledTime * (colors.Length - 1);
                    ...
                    {
                        float sizeTime = ScaledTime * (sizes.Length - 1);
